...
func ExampleNewExpvarCollector() {
	expvarCollector := dnxware.NewExpvarCollector(map[string]*dnxware.Desc{
		"memstats": dnxware.NewDesc(
			"expvar_memstats",
			"All numeric memstats as one metric family. Not a good role-model, actually... ;-)",
			[]string{"type"}, nil,
		),
		"lone-int": dnxware.NewDesc(
			"expvar_lone_int",
			"Just an expvar int as an example.",
			nil, nil,
		),
		"http-request-map": dnxware.NewDesc(
			"expvar_http_request_total",
			"How many http requests processed, partitioned by status code and http method.",
			[]string{"code", "method"}, nil,
		),
	})
	dnxware.MustRegister(expvarCollector)

	// The dnxware part is done here. But to show that this example is
	...
	// expvar what we want to re-export as dnxware metrics.
	...
	metric := dto.Metric{}
	metricChan := make(chan dnxware.Metric)
	go func() {
