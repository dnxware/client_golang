...
func (e errorCollector) Describe(ch chan<- *dnxware.Desc) {
	ch <- dnxware.NewDesc("invalid_metric", "not helpful", nil, nil)
}

func (e errorCollector) Collect(ch chan<- dnxware.Metric) {
	ch <- dnxware.NewInvalidMetric(
		dnxware.NewDesc("invalid_metric", "not helpful", nil, nil),
...
func (b blockingCollector) Describe(ch chan<- *dnxware.Desc) {
	ch <- dnxware.NewDesc("dummy_desc", "not helpful", nil, nil)
}

func (b blockingCollector) Collect(ch chan<- dnxware.Metric) {
	select {
...
func TestHandlerErrorHandling(t *testing.T) {
	...
	cnt := dnxware.NewCounter(dnxware.CounterOpts{
		Name: "the_count",
		Help: "Ah-ah-ah! Thunder and lightning!",
	})
	reg.MustRegister(cnt)

	cntVec := dnxware.NewCounterVec(
		dnxware.CounterOpts{
			Name:        "name",
			Help:        "docstring",
