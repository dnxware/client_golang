...
func TestLabelCheck(t *testing.T) {
	...
	for name, sc := range scenarios {
		t.Run(name, func(t *testing.T) {
			...
			o := dnxware.ObserverVec(dnxware.NewHistogramVec(
			...
			for _, l := range sc.curriedLabels {
				c = c.MustCurryWith(dnxware.Labels{l: "dummy"})
				o = o.MustCurryWith(dnxware.Labels{l: "dummy"})
			}
...
func TestMiddlewareAPI(t *testing.T) {
	...
	inFlightGauge := dnxware.NewGauge(dnxware.GaugeOpts{
	...
	histVec := dnxware.NewHistogramVec(
		dnxware.HistogramOpts{
			Name:        "response_duration_seconds",
			Help:        "A histogram of request latencies.",
			Buckets:     dnxware.DefBuckets,
			ConstLabels: dnxware.Labels{"handler": "api"},
		},
		[]string{"method"},
	)

	writeHeaderVec := dnxware.NewHistogramVec(
		dnxware.HistogramOpts{
...
func ExampleInstrumentHandlerDuration() {
	...
	duration := dnxware.NewHistogramVec(
		dnxware.HistogramOpts{
	...
	dnxware.MustRegister(inFlightGauge, counter, duration, responseSize)

	// Instrument the handlers with all the metrics, injecting the "handler"
	// label by currying.
	pushChain := InstrumentHandlerInFlight(inFlightGauge,
		InstrumentHandlerDuration(duration.MustCurryWith(dnxware.Labels{"handler": "push"}),
			InstrumentHandlerCounter(counter,
	...
	pullChain := InstrumentHandlerInFlight(inFlightGauge,
		InstrumentHandlerDuration(duration.MustCurryWith(dnxware.Labels{"handler": "pull"}),
			InstrumentHandlerCounter(counter,
