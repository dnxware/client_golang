...
// usually implemented by a dnxware.Registry) and pushes all gathered metrics
...
func FromGatherer(job string, grouping map[string]string, url string, g dnxware.Gatherer) error {
...
//
// Deprecated: Please use a Pusher created with New instead.
func AddFromGatherer(job string, grouping map[string]string, url string, g dnxware.Gatherer) error {
	return push(job, grouping, url, g, "POST")
}

func push(job string, grouping map[string]string, pushURL string, g dnxware.Gatherer, method string) error {
...
//
// Deprecated: Please use a Pusher created with New instead.
func Collectors(job string, grouping map[string]string, url string, collectors ...dnxware.Collector) error {
...
//
// Deprecated: Please use a Pusher created with New instead.
func AddCollectors(job string, grouping map[string]string, url string, collectors ...dnxware.Collector) error {
	return pushCollectors(job, grouping, url, "POST", collectors...)
}

func pushCollectors(job string, grouping map[string]string, url, method string, collectors ...dnxware.Collector) error {
...
// host-centric. (You would use https://github.com/dnxware/node_exporter in
// that case.) If you have the need to add the hostname to the grouping key, you
// are probably doing something wrong. See
// https://dnxware.io/docs/practices/pushing/ for details.
