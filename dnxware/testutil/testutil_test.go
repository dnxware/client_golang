...
func (u untypedCollector) Describe(c chan<- *dnxware.Desc) {
	c <- dnxware.NewDesc("name", "help", nil, nil)
}

func (u untypedCollector) Collect(c chan<- dnxware.Metric) {
	c <- dnxware.MustNewConstMetric(
		dnxware.NewDesc("name", "help", nil, nil),
		dnxware.UntypedValue,
		2001,
	)
}

func TestToFloat64(t *testing.T) {
	gaugeWithAValueSet := dnxware.NewGauge(dnxware.GaugeOpts{})
	gaugeWithAValueSet.Set(3.14)

	counterVecWithOneElement := dnxware.NewCounterVec(dnxware.CounterOpts{}, []string{"foo"})
	counterVecWithOneElement.WithLabelValues("bar").Inc()

	counterVecWithTwoElements := dnxware.NewCounterVec(dnxware.CounterOpts{}, []string{"foo"})
	counterVecWithTwoElements.WithLabelValues("bar").Add(42)
	counterVecWithTwoElements.WithLabelValues("baz").Inc()

	histogramVecWithOneElement := dnxware.NewHistogramVec(dnxware.HistogramOpts{}, []string{"foo"})
	...
	scenarios := map[string]struct {
		...
		"simple counter": {
			collector: dnxware.NewCounter(dnxware.CounterOpts{}),
			panics:    false,
			want:      0,
		},
		"simple gauge": {
			collector: dnxware.NewGauge(dnxware.GaugeOpts{}),
			panics:    false,
			want:      0,
		},
		"simple untyped": {
		...
		"simple histogram": {
			collector: dnxware.NewHistogram(dnxware.HistogramOpts{}),
			panics:    true,
		},
		"simple summary": {
			collector: dnxware.NewSummary(dnxware.SummaryOpts{}),
			panics:    true,
		},
		...
		"counter vec with zero elements": {
			collector: dnxware.NewCounterVec(dnxware.CounterOpts{}, nil),
			panics:    true,
		},
...
func TestCollectAndCompare(t *testing.T) {
	...
	c := dnxware.NewCounter(dnxware.CounterOpts{
		Name: "some_total",
		Help: "A value that represents a counter.",
		ConstLabels: dnxware.Labels{
			"label1": "value1",
		},
	})
	c.Inc()
...
func TestCollectAndCompareHistogram(t *testing.T) {
	inputs := []struct {
	...
	}{
		{
			...
			c: dnxware.NewHistogram(dnxware.HistogramOpts{
				Name:    "some_histogram",
		...
		},
		{
			...
			c: dnxware.NewHistogramVec(dnxware.HistogramOpts{
				Name:    "some_histogram",
				Help:    "An example of a histogram",
				Buckets: []float64{1, 2, 3},
			}, []string{"test"}),
	...
	for _, input := range inputs {
		...
		case dnxware.Histogram:
			collector.Observe(input.observation)
		case *dnxware.HistogramVec:
