...
// Package graphite provides a bridge to push dnxware metrics to a Graphite
// server.
package graphite
...
type Config struct {
	...
	// The Gatherer to use for metrics. Defaults to dnxware.DefaultGatherer.
	Gatherer dnxware.Gatherer
...
// Run starts the event loop that pushes dnxware metrics to Graphite at the
// configured interval.
...
// Push pushes dnxware metrics to the configured Graphite server.
