...
var (
	...
	// the dnxware server. Usually, you would track the duration of
	...
	apiRequestDuration = dnxware.NewHistogramVec(
		dnxware.HistogramOpts{
			Name:    "api_request_duration_seconds",
			Help:    "Histogram for the request duration of the public API, partitioned by status class.",
			Buckets: dnxware.ExponentialBuckets(0.1, 1.5, 5),
		},
		[]string{"status_class"},
	)
)
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	timer := dnxware.NewTimer(dnxware.ObserverFunc(func(v float64) {
		switch {
