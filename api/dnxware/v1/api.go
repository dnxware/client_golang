...
// Package v1 provides bindings to the dnxware HTTP API v1:
// http://dnxware.io/docs/querying/api/
package v1
...
// API provides bindings for dnxware's v1 API.
type API interface {
	// Alerts returns a list of all active alerts.
	Alerts(ctx context.Context) (AlertsResult, api.Error)
	// AlertManagers returns an overview of the current state of the dnxware alert manager discovery.
	...
	// Config returns the current dnxware configuration.
	...
	// Flags returns the flag values that dnxware was launched with.
	...
	// Targets returns an overview of the current state of the dnxware target discovery.
...
// ActiveTarget models an active dnxware scrape target.
...
// DroppedTarget models a dropped dnxware scrape target.
...
func apiError(code int) bool {
	// These are the codes that dnxware sends when it returns an error.
